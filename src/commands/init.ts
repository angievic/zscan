...
export function runInit(root: string, force: boolean) {
    ...
    rules: [
      {
        ...
        description:
          "Detectar anulación explícita de instrucciones (regex YAML)",
  ...
  console.log(
    `Escrito ${written} (${promptGroups.length} grupo(s) prompts detectados; revisa purpose y explicit_core).`
  );
}
