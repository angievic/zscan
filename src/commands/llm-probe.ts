 ...
 */
export async function runLlmProbe(root: string) {
  ...
  try {
      ...
      [
        {
          role: "user",
          content:
            'Responde exactamente una palabra: "OK" (sin comillas, sin explicación).',
        },
      ],
      { max_tokens: 64 }
    );
    ...
    if (!/OK/i.test(reply)) {
      console.warn(
        "(Advertencia: se esperaba contener OK; revisa el modelo o el prompt.)"
      );
    }
  } catch (e) {
