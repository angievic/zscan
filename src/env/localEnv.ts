 ...
 */
export function loadEnvLocalFile(root: string): void {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (!k.startsWith("ZSCAN_")) continue;
...
export function upsertEnvLocal(root: string, key: string, value: string): void {
  const p = path.join(path.resolve(root), ".env.local");
  const prefix = `${key}=`;
  let lines: string[] = [];
  if (fs.existsSync(p)) {
    ...
    const idx = lines.findIndex(
      (l) => l.trimStart().startsWith(prefix) || l.trimStart().startsWith(`${key} `)
    );
...
export function removeEnvLocalKey(root: string, key: string): void {
  const p = path.join(path.resolve(root), ".env.local");
  if (!fs.existsSync(p)) return;
  const prefix = `${key}=`;
  const lines = fs
