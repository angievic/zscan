...
function llmSnapshotForMarkdown(result: ScanResult): LlmUsageSnapshot {
  if (result.llmUsage) return result.llmUsage;
  ...
  if (!withVulns.length) {
    ...
    for (const f of withVulns) {
      ...
      for (const v of f.vulns) {
        ...
        lines.push(
          `- **${v.id}**${v.summary ? `: ${v.summary}` : ""} — _${sev.labelEs}_`
        );
...
export function ecosystemToMarkdown(eco: EcosystemScanResult): string {
...
export function toMarkdown(result: ScanResult): string {
  ...
  const pkgsConOsv = result.ecosystems.reduce(
  ...
  const envOv: string[] = [];
  if (llm.env.model) envOv.push("`ZSCAN_LLM_MODEL`");
  if (llm.env.base_url) envOv.push("`ZSCAN_LLM_BASE_URL`");
  if (llm.env.api_key) envOv.push("`ZSCAN_LLM_API_KEY`");
  if (llm.env.provider) envOv.push("`ZSCAN_LLM_PROVIDER`");
  lines.push(
    `- **Variables de entorno que pisan el YAML:** ${envOv.length ? envOv.join(", ") : "ninguna detectadas en este proceso"}`,
    "",
    "### Catálogo de referencia (proveedores soportados por zscan)",
    "",
    "Ejemplos documentados; el modelo en uso es el indicado arriba.",
    ""
  );
  for (const c of llm.catalogo_referencia) {
    lines.push(
      `- **${c.nombre}:** ${c.ejemplos_modelo.join(", ")}${c.nota ? ` — _${c.nota}_` : ""}`
    );
  }
  lines.push("", "## Resumen de vulnerabilidades", "");
  lines.push(
    "",
    vulnRes.alcanceLockfile,
    "",
    `- **Paquetes con versión consultada en OSV:** ${vulnRes.paquetesEscaneados}`,
    `- **Paquetes con al menos un hallazgo:** ${vulnRes.paquetesConVulnerabilidad}`,
    `- **IDs de vulnerabilidad únicos (GHSA/CVE/OSV):** ${vulnRes.idsVulnerabilidadUnicos.length}`,
    `- **Severidad (estándar):** ${vulnRes.estandarSeveridad} — bandas cualitativas según puntuación base cuando OSV la publica como número.`,
    "",
  );
  ...
  if (vulnRes.porVulnerabilidad.length === 0) {
    lines.push(
      "No hay vulnerabilidades conocidas en OSV para las versiones fijadas en este escaneo.",
      ""
    );
  } else {
    ...
    for (const row of vulnRes.porVulnerabilidad) {
      ...
      for (const p of row.enPaquetes) {
        lines.push(
          `- \`${p.package}@${p.version}\` · ${p.ecosystem} · \`${p.lockfile}\``
        );
      }
      lines.push("");
    }
  ...
  if (result.secretAuthScan) {
    ...
    lines.push(
      "## Secretos y autenticación (heurístico)",
      "",
      sa.metodo,
      "",
      `- **Nivel práctico (heurístico):** \`${sa.nivel}\``,
      "",
      sa.resumen,
      ""
    );
    if (sa.hallazgos.length) {
      ...
      for (const h of sa.hallazgos) {
        lines.push(
          `#### ${h.titulo}`,
          "",
          `- **Severidad:** ${h.severity} · **Categoría:** \`${h.category}\``,
          `- **Ubicación:** \`${h.path}:${h.line}\``,
          `- **Fragmento enmascarado:** \`${h.snippet}\``,
  ...
  if (meta?.enrichWebDiscovery?.length) {
    lines.push(
      "### Contexto web sugerido (OSV, NVD, búsqueda, foros)",
      "",
      "Enlaces para abrir en el navegador; zscan no consulta buscadores ni foros por ti, solo los lista.",
      ""
    );
    for (const b of meta.enrichWebDiscovery) {
      lines.push(
        `#### ${b.vulnId}${b.package ? ` · paquete \`${b.package}\`` : ""}`,
        ""
      );
