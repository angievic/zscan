...
export function findGemfileLock(root: string): string | null {
  const p = path.join(path.resolve(root), "Gemfile.lock");
  return fs.existsSync(p) ? p : null;
}

export function gemfileLockLabel(): string {
  return "Gemfile.lock";
}
 ...
 */
export function parseGemfileLockSpecs(text: string): LockfilePackage[] {
...
export function readGemfileLockPackages(root: string): LockfilePackage[] {
  const p = path.join(path.resolve(root), "Gemfile.lock");
