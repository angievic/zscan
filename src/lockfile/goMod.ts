...
export function findGoModFile(root: string): string | null {
  const p = path.join(path.resolve(root), "go.mod");
  return fs.existsSync(p) ? p : null;
}

export function goLockfileLabel(): string {
  return "go.mod";
}
...
export function parseGoModRequires(text: string): LockfilePackage[] {
  const out: LockfilePackage[] = [];
  const seen = new Set<string>();

  const push = (name: string, ver: string) => {
    const k = `${name}@${ver}`;
  ...
  while ((m = blockRe.exec(text)) !== null) {
    ...
    for (const raw of body.split("\n")) {
      ...
      const mod = parts[0]!;
      const ver = parts[1]!;
      if (mod === "go" || mod === "toolchain") continue;
      if (!ver.startsWith("v") && !/^[0-9]/.test(ver)) continue;
      push(mod, ver);
    }
  }
...
export function readGoModPackages(root: string): LockfilePackage[] {
  const p = path.join(path.resolve(root), "go.mod");
  const text = fs.readFileSync(p, "utf8");
  return parseGoModRequires(text);
}

export function readGoModulePath(root: string): string {
  const p = path.join(path.resolve(root), "go.mod");
  const text = fs.readFileSync(p, "utf8");
