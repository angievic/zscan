...
export function findPythonLockfile(root: string): PythonLockRef | null {
  const candidates: { kind: PythonLockKind; name: string }[] = [
...
export function parseTomlPackageBlocks(content: string): LockfilePackage[] {
...
export function readPipfileLock(lockPath: string): LockfilePackage[] {
  ...
  const ingest = (section?: Record<string, { version?: string }>) => {
    if (!section) return;
...
export function readRequirementsTxt(reqPath: string): LockfilePackage[] {
  const raw = fs.readFileSync(reqPath, "utf8");
  const seen = new Set<string>();
  const out: LockfilePackage[] = [];
  const lineRe =
    /^\s*([a-zA-Z0-9](?:[a-zA-Z0-9._-]*[a-zA-Z0-9])?)(?:\[[^\]]+\])?\s*==\s*([^#;\s]+)\s*(?:#.*)?$/;
...
export function readPythonLockfile(ref: PythonLockRef): LockfilePackage[] {
  switch (ref.kind) {
    case "poetry":
    case "uv": {
...
export function pythonLockfileLabel(kind: PythonLockKind): string {
  switch (kind) {
    case "poetry":
      return "poetry.lock";
    case "uv":
...
export function buildPythonFlatTree(
  projectLabel: string,
...
export function readPythonProjectName(root: string): string {
