...
function walkCodeFiles(rootAbs: string, out: string[]) {
  const scanRoots: string[] = [];
  const src = path.join(rootAbs, "src");
  if (fs.existsSync(src) && fs.statSync(src).isDirectory()) scanRoots.push(src);
...
function maskMiddle(s: string, keep = 6): string {
  const t = s.trim();
  if (t.length <= keep * 2) return t.slice(0, 3) + "…";
  return `${t.slice(0, keep)}…${t.slice(-keep)}`;
}

function pushFinding(
  out: SecretAuthFinding[],
  f: Omit<SecretAuthFinding, "snippet"> & { rawLine: string }
) {
  out.push({
    category: f.category,
    severity: f.severity,
...
function scanLine(
  ...
  lineNum: number,
  line: string,
  findings: SecretAuthFinding[]
) {
  const trimmed = line.trim();
  if (!trimmed || trimmed.startsWith("//") || trimmed.startsWith("#")) return;
  if (looksLikePlaceholder(line)) return;
  ...
  if (/-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----/.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "No versionar claves privadas: usá un gestor de secretos (Vault, cloud KMS), variables de entorno en runtime o archivos fuera del repo con .gitignore.",
    });
    return;
  }
  ...
  if (/\bAKIA[0-9A-Z]{16}\b/.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Rotá las claves expuestas, usá IAM roles / OIDC para CI y `aws configure` local sin pegar IDs en el repositorio.",
    });
    return;
  }

  if (
    /\b(?:aws_secret_access_key|AWS_SECRET_ACCESS_KEY)\s*[=:]\s*['"]?[A-Za-z0-9/+=]{20,}/i.test(
      line
    )
  ) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Tratá `AWS_SECRET_ACCESS_KEY` como material sensible: solo variables de entorno o secret store, nunca literal en git.",
    });
    return;
  }
  ...
  if (/\bghp_[a-zA-Z0-9]{20,}\b/.test(line) || /\bgithub_pat_[a-zA-Z0-9_]{20,}\b/.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Revocá el token en GitHub → Settings → Tokens y usá `GITHUB_TOKEN` en CI o credenciales del SO.",
    });
    return;
  }
  ...
  if (/\bsk-[a-zA-Z0-9]{20,}\b/.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Mové la clave a `.env.local` (ignorado) o al secret manager del proveedor; rotá si llegó a commitearse.",
    });
    return;
  }
  ...
  if (/\bBearer\s+[A-Za-z0-9._-]{24,}\b/.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Los tokens deben obtenerse en runtime (OAuth, sesión). Evitá literales en fuentes; si es de prueba, usá fixtures sin secretos reales.",
    });
    return;
  }
  ...
  if (basicUrl && basicUrl[2] && basicUrl[2].length > 2) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Usá variables de entorno, `.netrc` local ignorado o mecanismos del gestor de paquetes; las URLs con `user:pass@` suelen filtrarse en logs.",
    });
    return;
  }

  const apiKeyAssign =
    /\b(?:api[_-]?key|apikey|secret[_-]?key|client[_-]?secret)\s*[:=]\s*['"]([^'"\\]{12,})['"]/i.exec(
      line
    );
  if (apiKeyAssign && !/^(test|demo|fake|null|undefined)$/i.test(apiKeyAssign[1]!)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Sustituí por `process.env` / secret manager y validá que `.env*` esté en `.gitignore`.",
    });
    return;
  }

  const pwdAssign =
    /\bpassword\s*[:=]\s*['"]([^'"\\]{10,})['"]/i.exec(line);
  if (
    pwdAssign &&
    ...
    pushFinding(findings, {
      ...
      sugerencia:
        "Para apps: hash con bcrypt/argon2 en servidor; para clientes: flujos OAuth; nunca contraseñas en claro en repo.",
    });
    return;
  }
  ...
  if (/\bjwt\.sign\s*\([^)]*['"][^'"]{16,}['"]/i.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "El secreto de firma debe venir de variable de entorno (`JWT_SECRET`) con entropía fuerte y rotación planificada.",
    });
    return;
  }
  ...
  if (/\bhttp:\/\/[a-zA-Z0-9.-]+\.(?:api|auth|login|oauth)/i.test(line)) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Preferí `https://` para credenciales y tokens en tránsito; HTTP solo en desarrollo local explícito.",
    });
  }
}

function nivelFromFindings(findings: SecretAuthFinding[]): SecretAuthNivel {
  const high = findings.filter((f) => f.severity === "high").length;
  const med = findings.filter((f) => f.severity === "medium").length;
  const low = findings.filter((f) => f.severity === "low").length;
  if (high >= 1) return "riesgo_alto_heuristico";
  if (med >= 1 || low >= 2) return "revisar";
  if (low === 1 && findings.length === 1) return "sin_indicios_graves";
  if (findings.length >= 1) return "revisar";
  return "sin_indicios_graves";
}

function resumenTexto(nivel: SecretAuthNivel, findings: SecretAuthFinding[]): string {
  ...
  if (nivel === "sin_indicios_graves") {
    if (findings.length === 0) {
      return (
        "Heurísticas estáticas: no se detectaron patrones típicos de llaves/tokens en claro en los archivos analizados. " +
        "Eso no sustituye auditoría manual ni escaneo de secretos en historial git."
      );
    }
    return (
      `${findings.length} aviso menor (p. ej. presencia de .env o HTTP en desarrollo). ` +
      "Revisá la tabla; el nivel global queda bajo porque no hay indicios de alta severidad."
    );
  }
  if (nivel === "riesgo_alto_heuristico") {
    return (
      `Se hallaron ${h} indicio(s) de alta severidad (y ${m} medio(s), ${l} bajo(s)). ` +
      "Priorizá rotación de credenciales y eliminación del historial si ya se commitearon."
    );
  }
  return (
    `Hay ${findings.length} hallazgo(s) para revisar (${h} alto(s), ${m} medio(s), ${l} bajo(s)). ` +
 ...
 */
export function scanSecretsAndAuth(
  root: string,
  opts?: ScanWalkOptions & { git?: GitMetadata }
): SecretAuthScanResult {
  ...
  const findings: SecretAuthFinding[] = [];
  let analyzed = 0;
  ...
  if (fs.existsSync(envFile) && fs.statSync(envFile).isFile()) {
    pushFinding(findings, {
      ...
      sugerencia:
        "Confirmá que `.env` está en `.gitignore` y no en el remoto; si se versiona por error, rotá todos los valores sensibles.",
    });
  }

  const nivel = nivelFromFindings(findings);
  const metodo =
    "Análisis local por expresiones regulares sobre código y YAML (sin ejecutar la app). " +
