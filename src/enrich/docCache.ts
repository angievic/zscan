...
function ensureEnrichCacheDir(dir: string): boolean {
  try {
...
export function defaultEnrichCacheDir(): string {
  ...
  if (fromEnv && ensureEnrichCacheDir(fromEnv)) return fromEnv;

  const homeCache = path.join(os.homedir(), ".cache", "zscan", "enrich");
  if (ensureEnrichCacheDir(homeCache)) return homeCache;

  const tmpCache = path.join(os.tmpdir(), "zscan", "enrich");
  if (ensureEnrichCacheDir(tmpCache)) return tmpCache;

  const fallback = path.join(os.tmpdir(), `zscan-enrich-${process.pid}`);
  if (ensureEnrichCacheDir(fallback)) return fallback;

  throw new Error(
    "No se pudo crear caché de enriquecimiento; defina ZSCAN_ENRICH_CACHE_DIR a un directorio escribible."
  );
}
...
function normalizeOptions(
  options?: number | FetchUrlCachedOptions
): Required<FetchUrlCachedOptions> {
