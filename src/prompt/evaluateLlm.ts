...
import type { PromptFileResult, PromptRuleResult } from "./evaluate.js";

const MAX_PROMPT_CHARS = 12_000;

function tryParseEvaluationsJson(text: string): {
  evaluations: unknown[];
} {
  ...
  if (
    ...
    parsed === null ||
    !Array.isArray((parsed as { evaluations?: unknown }).evaluations)
  ) {
    throw new Error('JSON del modelo sin array "evaluations"');
  }
  return parsed as { evaluations: unknown[] };
}

function normalizeEvaluation(
  raw: unknown,
  ruleIds: Set<string>
): PromptRuleResult | null {
  if (typeof raw !== "object" || raw === null) return null;
  const o = raw as Record<string, unknown>;
  const ruleId = typeof o.ruleId === "string" ? o.ruleId.trim() : "";
  if (!ruleId || !ruleIds.has(ruleId)) return null;
  ...
  const ruleDescription =
    typeof o.ruleDescription === "string"
...
function recalcFileScore(file: PromptFileResult): void {
 ...
 */
export async function appendLlmRuleChecks(
  fileResult: PromptFileResult,
  content: string,
  cfg: ZscanConfig,
  llm: ResolvedLlmOptions
  ...
  let text: string;
  try {
    text = await invokeChat(
      llm,
      [
        {
          role: "system",
          content:
            "Eres un auditor de prompts y documentación técnica. Respondes solo con JSON válido UTF-8, sin markdown ni texto fuera del objeto.",
        },
