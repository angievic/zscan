...
export function createBundleDirName(): string {
 ...
 */
export function writeScanBundle(parentDir: string, result: ScanResult): string {
  ...
  fs.writeFileSync(
    path.join(bundleDir, "informe.json"),
    prettyJson(informeView),
    "utf8"
  );
  fs.writeFileSync(
    path.join(bundleDir, "prompts.json"),
    prettyJson(buildPromptsInformeJson(result)),
    "utf8"
  );
  ...
  if (result.prompts) {
    fs.writeFileSync(
      path.join(bundleDir, "prompts.md"),
      promptScanToMarkdown(result.prompts, { embedInScanReport: false }),
      "utf8"
    );
