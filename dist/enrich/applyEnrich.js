...
function mergeOsvVuln(thin, detail) {
    if (!detail)
        return thin;
...
function osvPublicPageUrl(vulnId) {
 ...
 */
export async function applyDocEnrichment(result, options) {
    ...
    const fetchOpts = { timeoutMs: 24_000, retries: 2 };
    if (!result.meta)
        result.meta = {};
    ...
    function recordDiscovery(vulnId, pkg) {
        if (discoveryIds.has(vulnId))
            return;
        ...
        for (const f of eco.findings) {
            for (const v of f.vulns) {
                ...
                for (const url of tryUrls) {
                    if (snippetCount >= maxSnippets)
                        break outer;
                    if (attempts >= maxAttemptsPerVuln)
                        break;
                    attempts++;
                    try {
                ...
                if (!got && attempts > 0) {
                    meta.enrichErrors.push(`${v.id}: sin extracto tras ${attempts} URL(s) (${lastFail ?? "error desconocido"}). Usá “Contexto web” en el informe.`);
                }
            }
        }
    }
    ...
    if (meta.enrichErrors?.length === 0)
        delete meta.enrichErrors;
    if (meta.docSnippets?.length === 0)
        delete meta.docSnippets;
    if (meta.enrichWebDiscovery?.length === 0)
        delete meta.enrichWebDiscovery;
}
