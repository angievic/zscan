...
import { evaluatePromptContent, mergePromptScanMeta, } from "./evaluate.js";
import { appendLlmRuleChecks } from "./evaluateLlm.js";
import { expandPromptPaths } from "./expandPaths.js";
export async function runPromptScanCore(root, options) {
    const abs = path.resolve(root);
    loadEnvLocalFile(abs);
    const cfg = loadConfig(abs);
    const llmOpts = resolveLlmOptions(cfg);
    const useLlmForScan = llmOpts.enabled && options?.skipLlm !== true;
    const groups = cfg.prompts ?? [];
    const minRequired = cfg.reliability?.prompts_min_percent ?? 85;
    if (!groups.length) {
        return {
            result: null,
            error: "No hay entradas en `prompts[]` de zscan.yaml; añade paths y `purpose`, o usa `zscan init`.",
        };
    }
    const skippedYamlRules = [];
    const patternErrors = [];
    const llmErrors = [];
    const files = [];
    for (const group of groups) {
        ...
        for (const fileAbs of resolved) {
            ...
            const { fileResult, skipped, patternErrors: pe } = evaluatePromptContent(fileAbs, rel, group.purpose ?? "(sin purpose en YAML)", content, cfg, { llmCoversRulesWithoutPattern: useLlmForScan });
            if (useLlmForScan && (cfg.rules?.length ?? 0) > 0) {
                const llmErr = await appendLlmRuleChecks(fileResult, content, cfg, llmOpts);
                if (llmErr)
                    llmErrors.push(`${rel}: ${llmErr}`);
            }
    ...
    if (!files.length) {
        return {
            result: null,
            error: "Los globs de `prompts[]` no coincidieron con ningún archivo (revisa rutas y .gitignore implícito).",
        };
    }
