...
const PLACEHOLDER_RES = /\{\{[\s\S]*?\}\}|\$\{[^}]+\}|%\s*\w+\s*%|<%[\s\S]*?%>|__\w+__/;
function analyzeLine(line, lineNum) {
    ...
    let best = null;
    const consider = (d) => {
        if (!best || rank(d.sensibilidad) > rank(best.sensibilidad))
            best = d;
    };
    for (const x of ALTA) {
        if (x.re.test(line)) {
    ...
    if (PLACEHOLDER_RES.test(line)) {
        consider({
            line: lineNum,
            hallazgo: "Marcador de plantilla ({{…}}, ${…}, etc.): el valor real depende del código que rellena el prompt",
            sensibilidad: "media",
...
function uniqueTipos(detalles) {
    const s = new Set();
    for (const d of detalles)
        s.add(d.tipo_inferido);
    return [...s].sort((a, b) => a.localeCompare(b));
}
function buildSugerencias(nivel, tieneSustituciones, tipos) {
    const out = [];
    if (nivel === "alta" || tipos.some((t) => /credencial|secreto|token|salud|financier/i.test(t))) {
        out.push("Evitá incluir secretos, tokens reales o PII en el texto estático del prompt; usá variables de entorno o vault solo en el servidor.");
        out.push("Si el modelo recibe datos regulados (salud, financieros), revisá acuerdos con el proveedor del LLM y minimización de datos.");
    }
    if (nivel === "media" || tieneSustituciones) {
        out.push("Para sustituciones: validá y acotá longitud del texto de usuario; considerá filtrado previo (DLP, regex de PII) antes de llamar al modelo.");
        out.push("Registrá qué campos se concatenan al prompt en código; los logs de proveedor pueden retener el contenido enviado.");
    }
    if (nivel === "baja" && !tieneSustituciones) {
        out.push("Prompt mayormente estático: el riesgo principal suele ser si en runtime se le añaden datos sin revisar (revisá el código que arma el mensaje).");
    }
    if (nivel === "indeterminado") {
        out.push("No hubo patrones claros: revisá manualmente qué variables inyecta tu aplicación en este template.");
    }
    return out;
}
export function analyzePromptDataSensitivity(content) {
