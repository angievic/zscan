...
export function collectImportHints(root, packages, opts) {
...
function moduleAliases(pkgName) {
    const out = new Set();
...
export function collectPythonImportHints(root, packages, opts) {
    const rootAbs = path.resolve(root);
    const files = [];
    const src = path.join(rootAbs, "src");
    const start = fs.existsSync(src) ? src : rootAbs;
...
function extractGoImportStrings(content) {
    const out = [];
    ...
    const lineRe = /^\s*import\s+(?:\.\s+|_\s+|\w+\s+)?"((?:\\.|[^"\\])*)"/gm;
    while ((m = lineRe.exec(content)) !== null) {
        out.push(m[1].replace(/\\"/g, '"'));
    }
    return out;
}
function longestGoModuleMatch(importPath, modules) {
    let best = null;
    let bestLen = -1;
...
export function collectGoImportHints(root, packages, opts) {
    const rootAbs = path.resolve(root);
    ...
    const filtered = filterSubmoduleAbsPaths(files, opts);
    const byPkg = new Map();
    const pushHit = (mod, rel, lineNo, snippet) => {
...
function rubyRequireFragments(line) {
    const out = [];
    const re = /\brequire(?:_dependency)?\s+["']([^"']+)["']/g;
    let m;
...
function gemRequireCandidates(gemName) {
    const s = new Set();
...
function resolveRubyGem(requirePath, packages) {
...
export function collectRubyImportHints(root, packages, opts) {
    const rootAbs = path.resolve(root);
    const files = [];
    const src = path.join(rootAbs, "src");
    const start = fs.existsSync(src) ? src : rootAbs;
...
function javaImportLines(content) {
    const out = [];
    const lines = content.split(/\r?\n/);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m)
            out.push({ line: i + 1, fqcn: m[1] });
    }
    return out;
}
function mavenCoords(p) {
...
export function collectJavaImportHints(root, packages, opts) {
    const rootAbs = path.resolve(root);
