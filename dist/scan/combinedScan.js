...
import { performProjectScan, } from "./projectScan.js";
/** `true` si el mensaje indica que no hay nada que evaluar (no es fallo del pipeline). */
function isPromptScanOptionalSkip(message) {
 ...
 */
export async function performCombinedScan(root, depOpts, promptOpts) {
    ...
    if (pr) {
        ...
        if (!isPromptScanOptionalSkip(pe))
            promptScanHardFail = true;
    }
...
export function promptChecksFailed(result) {
    return Boolean(result.prompts?.files.some((f) => f.checks.some((c) => !c.passed)));
}
