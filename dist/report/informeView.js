...
import { CVSS_QUALITATIVE_ORDER, classifyOsvSeverities, qualitativeShortEs, worseClassified, } from "./cvssSeverity.js";
export const ZSCAN_INFORME_VERSION = 5;
/** Texto fijo: el escaneo cubre todo el lockfile, no solo dependencias directas ni el “framework”. */
export const INFORME_ALCANCE_LOCKFILE = "zscan compara contra OSV las versiones de cada paquete listado en el lockfile (directas y transitivas). Los hallazgos no se limitan al framework ni al nombre del proyecto: utilidades, plugins, tipos y dependencias anidadas entran en el mismo análisis.";
export const ZSCAN_PROMPTS_INFORME_VERSION = 1;
export function prettyJson(value) {
    return `${JSON.stringify(value, null, 2)}\n`;
}
function countVulnPkgs(eco) {
    return eco.findings.filter((f) => f.vulns?.length).length;
}
function flatHallazgos(ecosystems) {
    const rows = [];
...
export function buildVulnResumen(ecosystems, numPaquetes, numPaquetesConOsv) {
    const byId = new Map();
    for (const eco of ecosystems) {
        for (const f of eco.findings) {
            if (!f.vulns?.length)
    ...
    const conteoPorSeveridad = CVSS_QUALITATIVE_ORDER.filter((q) => (conteoMap.get(q) ?? 0) > 0).map((qualitative) => ({
        qualitative,
...
export function buildInformeView(result) {
    ...
    const llm = result.llmUsage ?? buildLlmUsageSnapshot(loadConfig(result.root));
    const m = result.meta;
    const metaResumen = m
        ? {
            offline: m.offline,
...
export function buildPromptsInformeJson(result) {
    return {
