...
export function parseYarnV1DescriptorKey(key) {
    const k = key.trim();
...
export function parseYarnBerryKey(key) {
    const k = key
        .trim()
        .replace(/^"/, "")
        .replace(/"$/, "")
...
function readYarnClassic(content) {
    ...
    while (i < lines.length) {
        ...
        const keys = header.split(",").map((k) => k
            .trim()
            .replace(/^"/, "")
            .replace(/"$/, ""));
        i++;
...
function readYarnBerry(content) {
    const lines = content.split(/\r?\n/);
    const seen = new Set();
    const out = [];
    let i = 0;
    while (i < lines.length) {
        ...
        if (headerTrim.includes("@workspace:") ||
        ...
        i++;
        let version = berry?.version ?? null;
        let name = berry?.name ?? null;
        while (i < lines.length && /^\s/.test(lines[i])) {
            const vm = lines[i].match(/^\s+version:\s*(?:"([^"]+)"|([0-9][^#\s]*))/);
            if (vm)
                version = vm[1] ?? vm[2] ?? version;
...
export function readYarnLockfile(lockPath) {
