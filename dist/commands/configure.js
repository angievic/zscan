...
import { ZSCAN_ANTHROPIC_BASE_URL_DEFAULT, ZSCAN_CLAUDE_MODEL_DEFAULT, ZSCAN_GEMINI_MODEL_DEFAULT, ZSCAN_GEMINI_OPENAI_BASE_URL, ZSCAN_LLM_BASE_URL_DEFAULT, ZSCAN_LLM_MODEL_DEFAULT, } from "../llm/constants.js";
...
function applyApiKeyToEnvLocal(absRoot, apiKey) {
 ...
 */
export async function runInteractiveConfig(root) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error("zscan config necesita una terminal interactiva (TTY). En CI usa variables ZSCAN_LLM_* y edita zscan.yaml a mano.");
        process.exitCode = 1;
        return;
    }
    ...
    while (!["1", "2", "3", "4"].includes(choice)) {
        const raw = (await rl.question("Proveedor:  [1] OpenAI   [2] Ollama (local)   [3] Google Gemini   [4] Anthropic Claude   [2]: ")).trim();
        choice = raw === "" ? "2" : raw;
    ...
    if (choice === "1") {
        ...
        const model = (await rl.question(`Modelo [${OPENAI_MODEL_DEFAULT}]: `)).trim() || OPENAI_MODEL_DEFAULT;
        applyApiKeyToEnvLocal(absRoot, apiKey);
        patchYamlLlmSection(absRoot, { enabled: true, base_url: OPENAI_BASE_URL, model }, { removeApiKeyFromYaml: true, dropProviderField: true });
        ...
        const model = (await rl.question(`Modelo Gemini [${ZSCAN_GEMINI_MODEL_DEFAULT}]: `)).trim() || ZSCAN_GEMINI_MODEL_DEFAULT;
        applyApiKeyToEnvLocal(absRoot, apiKey);
        patchYamlLlmSection(absRoot, {
            enabled: true,
        ...
        const model = (await rl.question(`Modelo Claude [${ZSCAN_CLAUDE_MODEL_DEFAULT}]: `)).trim() || ZSCAN_CLAUDE_MODEL_DEFAULT;
        applyApiKeyToEnvLocal(absRoot, apiKey);
        patchYamlLlmSection(absRoot, {
            enabled: true,
        ...
        const model = (await rl.question(`Modelo Ollama [${ZSCAN_LLM_MODEL_DEFAULT}]: `)).trim() || ZSCAN_LLM_MODEL_DEFAULT;
    ...
    rl.close();
    console.log("Asegúrate de tener `.env.local` en `.gitignore` si guardas claves ahí.\n");
    loadEnvLocalFile(absRoot);
