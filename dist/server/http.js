...
export function createZscanServer() {
    return http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === "POST" && url.pathname === "/scan") {
                ...
                const promptChecksFailed = Boolean(result.prompts?.files.some((f) => f.checks.some((c) => !c.passed)));
                sendJson(res, 200, {
                    ok: true,
                    vulnCount,
                    promptScanHardFail,
...
export function startZscanServer(opts) {
