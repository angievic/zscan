...
function chunkCachePath(chunk, ecosystem, cacheDir) {
...
function readCachedBatch(filePath) {
    try {
...
function writeCachedBatch(filePath, results) {
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, JSON.stringify({ results }), "utf8");
}
export async function queryOsvBatch(packages, ecosystem = "npm", opts) {
    const out = new Map();
    ...
    for (let i = 0; i < packages.length; i += chunkSize) {
        ...
        if (offline) {
            ...
            if (!cached || cached.results.length !== chunk.length) {
                warn(`OSV offline: sin caché para un lote de ${chunk.length} paquetes (${ecosystem}); se asume sin CVE en caché.`);
        ...
        try {
            writeCachedBatch(cacheFile, chunk.map((_, j) => ({ vulns: data.results[j]?.vulns ?? [] })));
        }
        catch (e) {
 ...
 */
export async function fetchOsvVulnDetail(id) {
    try {
